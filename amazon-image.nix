...

{
  ...
  imports = (pkgs.path + /nixos/modules/virtualisation/) ++ [ ../profiles/headless.nix ./ec2-data.nix ./amazon-init.nix ];
  ...
  config = {
    ...
    boot.loader.grub.device = if (cfg.hvm && !cfg.efi) then "/dev/xvdz" else "nodev";
    ...
    boot.initrd.postMountCommands =
      ''
        ...
        for device in /dev/xvd[abcde]*; do
            if [ "$device" = /dev/xvdz -o "$device" = /dev/xvdz1 ]; then continue; fi
